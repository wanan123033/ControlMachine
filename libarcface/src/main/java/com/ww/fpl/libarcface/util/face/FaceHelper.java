...
//        flEngine = builder.flEngine;
        mHeight = builder.iHeight;
        mWidth = builder.iWidth;
        /**
         * fr 线程队列大小
         */
        int frQueueSize = 5;
...
//                long ftStartTime = System.currentTimeMillis();
                int code = ftEngine.detectFaces(nv21, previewSize == null ? mWidth : previewSize.width, previewSize == null ? mHeight : previewSize.height, FaceEngine.CP_PAF_NV21, faceInfoList);
                if (code != ErrorInfo.MOK) {
                    faceListener.onFail(new Exception("ft failed,code is " + code));
                } else {
//                    Log.i(TAG, "onPreviewFrame: ft costTime = " + (System.currentTimeMillis() - ftStartTime) + "ms"+"---"+faceInfoList.size());
                }
                /*
                 * 若需要多人脸搜索，删除此行代码
                 */
