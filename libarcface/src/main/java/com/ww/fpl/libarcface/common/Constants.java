package com.ww.fpl.libarcface.common;

public class Constants {

    public static final String APP_ID = "41af27gHWLS7GhSG7d2yJkbG1814NKGq3Y9CRRXKdu6x";
    public static final String SDK_KEY = "755jfJ8xkCr9gNGExKnAh5KWiz3CwUCyt5qd2L8r4faV";

    /**
     * IR预览数据相对于RGB预览数据的横向偏移量，注意：是预览数据，一般的摄像头的预览数据都是 width > height
     */
    public static final int HORIZONTAL_OFFSET = 0;
    /**
     * IR预览数据相对于RGB预览数据的纵向偏移量，注意：是预览数据，一般的摄像头的预览数据都是 width > height
     */
    public static final int VERTICAL_OFFSET = 0;
}

