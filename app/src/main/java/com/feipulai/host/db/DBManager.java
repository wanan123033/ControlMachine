...
public class DBManager{
	 ...
	 * @return
	 */
	public List<Student> getChooseStudentList(String itemCode, boolean isTested, boolean isUnTested,
		...
		sb.append("SELECT S.*  FROM " +
				StudentDao.TABLENAME + " S LEFT JOIN " + StudentItemDao.TABLENAME + " I " +
				" ON S." + StudentDao.Properties.StudentCode.columnName + " = I." + StudentItemDao.Properties.StudentCode.columnName +
				" WHERE I." + StudentItemDao.Properties.MachineCode.columnName + " = " + TestConfigs.sCurrentItem.getMachineCode() +
				" AND I." + StudentItemDao.Properties.ItemCode.columnName + " = '" + itemCode + "'" +
				" AND S." + StudentDao.Properties.StudentCode.columnName);
		boolean needWhere = true;
		if (isTested) {
			sb.append(" IN (SELECT " + RoundResultDao.Properties.StudentCode.columnName + " FROM " + RoundResultDao.TABLENAME + " R");
		} else if (isUnTested) {
			sb.append(" NOT IN (SELECT " + RoundResultDao.Properties.StudentCode.columnName + " FROM " + RoundResultDao.TABLENAME + " R");
		}
		boolean inClauseAppended = isTested || isUnTested;
		if (isUpload) {
			if (inClauseAppended) {
				sb.append(" WHERE R." + RoundResultDao.Properties.UpdateState.columnName + "= 1  AND ");
			} else {
				sb.append(" IN (SELECT " + RoundResultDao.Properties.StudentCode.columnName + " FROM " + RoundResultDao.TABLENAME +
						" R  WHERE R." + RoundResultDao.Properties.UpdateState.columnName + "= 1 AND ");
			}
			needWhere = false;
		} else if (isUnUpload) {
			if (inClauseAppended) {
				sb.append(" WHERE R." + RoundResultDao.Properties.UpdateState.columnName + "= 0 AND ");
			} else {
				sb.append(" IN (SELECT " + RoundResultDao.Properties.StudentCode.columnName + " FROM " + RoundResultDao.TABLENAME +
						" R  WHERE R." + RoundResultDao.Properties.UpdateState.columnName + "= 0 AND ");
			}
			needWhere = false;
		}
		if (needWhere) {
		...
		sb.append(" R." + RoundResultDao.Properties.ItemCode.columnName + " = '" + itemCode + "' " +
				"AND R." + RoundResultDao.Properties.MachineCode.columnName + " = " + TestConfigs.sCurrentItem.getMachineCode() +
				") LIMIT " + limit + " OFFSET " + offset);
		Logger.i("getChooseStudentList:" + sb.toString());
		
		Cursor c = db.rawQuery(sb.toString(),null);
		List<Student> students = new ArrayList<>();
		while(c.moveToNext()){
			students.add(studentDao.readEntity(c,0));
		}
		return students;
	}
	...
	public Map<String, Object> getChooseStudentCount(boolean isTested, boolean isUnTested, boolean isUpload, boolean isUnUpload) {
		Logger.i("zzs===>" + isTested + "---" + isUnTested + "---" + isUpload + "---" + isUnUpload);
		StringBuilder sqlBuf = new StringBuilder("SELECT COUNT(*) AS STU_COUNT,");
