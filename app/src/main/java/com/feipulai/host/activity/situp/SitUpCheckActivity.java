...
public class SitUpCheckActivity
	...
	public void onCheckIn(Student stu){
		...
		mLEDManager.showString(hostId,stu.getStudentName(),5,0,true,false);
		
		if(lastResult == null){
			mLEDManager.showString(hostId,deviceState.getDeviceId() + "号设备",4,1,false,true);
		}else{
			mLEDManager.showString(ItemDefault.CODE_YWQZ,hostId,deviceState.getDeviceId() + "号设备",4,1,false,false);
			mLEDManager.showString(ItemDefault.CODE_YWQZ,hostId,"已有成绩:" + mTvGrade.getText(),2,3,false,true);
		}
	...
	private void checkConnectingDevice(SitPushUpSetFrequencyResult result){
		...
		if(mCurrentFrequency == 0){
			...
			if(result.getFrequency() == mTargetFrequency && result.getDeviceId() == mCurrentPosition + 1){
				...
				mSitPushUpManager.setFrequency(projectCode,result.getFrequency(),mCurrentPosition + 1,hostId);
				mCurrentFrequency = mTargetFrequency;
				// 那个铁盒子就是有可能等这么久才收到回复
