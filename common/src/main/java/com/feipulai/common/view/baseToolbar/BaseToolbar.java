...
public class BaseToolbar extends Toolbar {
    ...
    public int getViewLayoutParamsMargin() {
        int margin = Math.max(mLayoutLeft.getWidth(), mLayoutRight.getWidth());
        if (margin > DisplayUtil.getScreenWidth(getContext()) / 2) {
     ...
     * @param colorId
     */
    public void setToolbarBackgroundColor(@ColorInt int colorId) {
    ...
    public static class Builder {
        ...
        public BaseToolbar build() {
            ...
            toolbar.setToolbarBackgroundColor(backgroundColorId);
    ...
    public static TextView createTextMenu(Context context, CharSequence text, @ColorInt int textColorId, OnClickListener listener) {
        return createTextMenu(context, text, textColorId, 18, listener);
    }
