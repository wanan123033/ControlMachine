...
public class QRReadRunnable extends SerialReadRunnable {
    ...
    public void convert(Message msg) {
        ...
        try {
            ...
            if (SerialParams.QR_CODE.getVersions() == 3 && qrCodeBytes[size - 1] == 0X0D) {
                qrCodeBytes = new byte[size - 1];
                System.arraycopy(data, 0, qrCodeBytes, 0, size - 1);
            }
            Log.i(TAG, "convert: size: " + StringUtility.bytesToHexString(qrCodeBytes));
