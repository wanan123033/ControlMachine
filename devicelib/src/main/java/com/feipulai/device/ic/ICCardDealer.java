...
public class ICCardDealer{
	...
	public StuInfo IC_ReadStuInfo(){
		byte[] stuData = readBlocks(ICBlockIndex.STU_INFO_BLOCK_NO);
		if(stuData != null){
	 ...
	 * 读取的项目成绩单位为IC卡中的单位,与数据库中取出的数据进行比较需要先进行单位转换,具体可以参考 {@link com.feipulai.exam.ic.utils.ICResultConverter}
	 ...
	 *                    在写入成绩到IC卡中之前,需要将成绩转换为IC卡中的单位格式,具体参考 {@link com.feipulai.host.ic.utils.ICResultConverter}
