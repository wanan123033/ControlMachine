...
public class ControllerCover extends BaseCover implements OnTimerUpdateListener, OnTouchGestureListener, View.OnClickListener, RadioGroup.OnCheckedChangeListener {
...
//        setSpeedContainerState(state);
    ...
    public void onCheckedChanged(RadioGroup group, int checkedId) {
        if (checkedId == R.id.rb_speed_half) {
            notifyReceiverEvent(DataInter.Event.EVENT_CODE_REQUEST_SPEED_X_HALF, null);
        } else if (checkedId == R.id.rb_speed_1) {
            notifyReceiverEvent(DataInter.Event.EVENT_CODE_REQUEST_SPEED_X_1, null);
        } else if (checkedId == R.id.rb_speed_2) {
            notifyReceiverEvent(DataInter.Event.EVENT_CODE_REQUEST_SPEED_X_2, null);
        }
    }
}
