...
//            InteractUtils.toast(this, "读卡失败");
    ...
    private static class MyHandler extends Handler {
        ...
        private WeakReference<BaseCheckMiddleActivity> mReference;

        public MyHandler(BaseCheckMiddleActivity reference) {
            mReference = new WeakReference<>(reference);
        }

        @Override
        public void handleMessage(Message msg) {
            BaseCheckMiddleActivity activity = mReference.get();
            if (activity == null) {
                return;
            }
            switch (msg.what) {
    ...
    private void showAddHint(final Student student) {
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                new SweetAlertDialog(BaseCheckMiddleActivity.this).setTitleText(getString(R.string.addStu_dialog_title))
                        .setContentText(getString(R.string.addStu_dialog_content))
                        .setConfirmText(getString(R.string.confirm)).setConfirmClickListener(new SweetAlertDialog.OnSweetClickListener() {
                    @Override
                    public void onClick(SweetAlertDialog sweetAlertDialog) {
                        sweetAlertDialog.dismissWithAnimation();
                        new AddStudentDialog(BaseCheckMiddleActivity.this, MiddleDistanceRaceForPersonActivity.itemCode).showDialog(student, false);
                    }
