...
public class FootballIndividualActivity extends BaseTitleActivity implements IndividualCheckFragment.OnIndividualCheckInListener,
        TimerUtil.TimerAccepListener, BasketBallListener.BasketBallResponseListener {
        ...
        switch (useMode) {
            ...
            case 1://"2:起点1:终点"
                if (result.gettNum() == 1) {//拦截到了起点
                    tvResult.setText(DateUtil.caculateFormatTime(result.getResult(), TestConfigs.sCurrentItem.getDigital() == 0 ?
                            2 : TestConfigs.sCurrentItem.getDigital()));
                } else {//拦截到终点，正常
                    doGetResult(result);
                }
...
//            DBManager.getInstance().updateRoundResult(dbAscResult);
    ...
    public void timer(Long time) {
        if (state == TESTING) {
            tvResult.setText(DateUtil.caculateTime(time * 10, TestConfigs.sCurrentItem.getDigital() == 0 ? 2 :
                    TestConfigs.sCurrentItem.getDigital(), 0));
        }
    }
