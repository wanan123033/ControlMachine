...
public class VerticalStepViewIndicator extends View {
    ...
    private int mUnCompletedLineColor = ContextCompat.getColor(getContext(), R.color.grey_light);//定义默认未完成线的颜色  definition mUnCompletedLineColor
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        for (int i = 0; i < mStepNum; i++) {
            ...
            if (mIsReverseDraw) {
                mCircleCenterPointPositionList.add(mHeight - (mCircleRadius + i * mCircleRadius * 2 + i * mLinePadding) - getPaddingBottom());
            } else {
                mCircleCenterPointPositionList.add(mCircleRadius + i * mCircleRadius * 2 + i * mLinePadding + getPaddingTop());
            }
        }
        /**
         * set listener
