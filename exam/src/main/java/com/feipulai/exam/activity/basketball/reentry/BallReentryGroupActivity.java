...
public class BallReentryGroupActivity extends BaseTitleActivity implements BasketBallListener.BasketBallResponseListener, TimerUtil.TimerAccepListener, BaseQuickAdapter.OnItemClickListener {
...
//        timerUtil.release();
        ...
        resultAdapter.notifyDataSetChanged();
        LogUtils.operation("篮球保存机器拦截成绩:" + machineResult.toString());
...
//                break;
//        }
//        if (chronometer - topChronometer >= 80) {
//
//            topChronometer = chronometer;
//        }
        timerDate = time * 70;
        if (state == TESTING) {
        ...
        switch (view.getId()) {
            case R.id.tv_reentry_add:
                new SweetAlertDialog(this, SweetAlertDialog.WARNING_TYPE).setTitleText("温馨提示")
                    ...
                    public void onClick(SweetAlertDialog sweetAlertDialog) {
                        ...
                        machineResult.setMachineCode(TestConfigs.sCurrentItem.getMachineCode());
                        machineResult.setTestNo(1);
                        machineResult.setRoundNo(resultAdapter.getSelectPosition() + 1);
                        machineResult.setStudentCode(student.getStudentCode());
            ...
            case R.id.tv_reentry_remove:
                ...
                new SweetAlertDialog(this, SweetAlertDialog.WARNING_TYPE).setTitleText("温馨提示")
                    ...
                    public void onClick(SweetAlertDialog sweetAlertDialog) {
                        sweetAlertDialog.dismissWithAnimation();
                        DBManager.getInstance().deleteStuMachineResultsReentry(student.getStudentCode(), 1, resultAdapter.getSelectPosition() + 1, group.getId());
                        ...
                        switch (which) {
                            ...
                            case 1:
                                ...
                                InteractUtils.printResults(group, testCache.getAllStudents(), testCache.getResults(),
                                        TestConfigs.getMaxTestCount(BallReentryGroupActivity.this), testCache.getTrackNoMap());
                                break;
                        }
                    }
                }).create().show();
    ...
    private void presetResult() {
        ...
        for (int i = 0; i < setTestCount(); i++) {
            ...
            if (roundResult == null) {
                ...
                MachineResult reentry = DBManager.getInstance().getGroupMachineResultReentry(student.getStudentCode(),
                        group.getId(), i + 1);
...
//                resultList.add(new BasketBallTestResult(i + 1, machineResultList, roundResult.getMachineResult(), roundResult.getResult(), roundResult.getPenaltyNum(), roundResult.getResultState()));
        ...
        resultAdapter.notifyDataSetChanged();
        if (resultList.get(resultAdapter.getSelectPosition()).getReentry() >= 0) {
            tvReentryRemove.setVisibility(View.VISIBLE);
            tvReentryAdd.setVisibility(View.GONE);
        } else {
            tvReentryRemove.setVisibility(View.GONE);
...
//                UdpLEDUtil.getLedByte("", Paint.Align.RIGHT)));
    ...
    private void showBeginLed(final String stuName) {
