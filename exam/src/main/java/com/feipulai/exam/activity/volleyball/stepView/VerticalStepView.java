package com.feipulai.exam.activity.volleyball.stepView;
...
public class VerticalStepView extends LinearLayout implements VerticalStepViewIndicator.OnDrawIndicatorListener {
    ...
    private int mUnComplectedTextColor = ContextCompat.getColor(getContext(), R.color.viewfinder_laser);//定义默认未完成文字的颜色;
    ...
    public VerticalStepView setStepViewTexts(List<StepBean> stepsBeanList) {
        ...
        mStepsViewIndicator.setStepList(mStepBeanList);
        return this;
    }

    public VerticalStepView setCircleRadius(float circleRadius) {
        mStepsViewIndicator.setCircleRadius(circleRadius);
        return this;
    }

    public VerticalStepView setDefaultStepIndicatorNum(int defaultStepIndicatorNum) {
        mStepsViewIndicator.setDefaultStepIndicatorNum(defaultStepIndicatorNum);
        return this;
    }
    /**
     * 设置显示的文字
...
//    }
    ...
    public VerticalStepView setStepBackgroundResource(int backgroundResource) {
        mStepsViewIndicator.setBackgroundResource(backgroundResource);
        return this;
    }
        ...
        if (mTextContainer != null) {
            ...
            if (mStepBeanList != null && complectedXPosition != null && complectedXPosition.size() > 0) {
                for (int i = 0; i < mStepBeanList.size(); i++) {
                    ...
                    if (!TextUtils.isEmpty(mStepBeanList.get(i).getName())) {
                        Drawable drawable = ContextCompat.getDrawable(getContext(), R.mipmap.icon_red_right_arrows);
                        drawable.setBounds(0, 0, drawable.getMinimumWidth(), drawable.getMinimumHeight());
                        mTextView.setCompoundDrawables(drawable, null, null, null);
                        mTextView.setCompoundDrawablePadding(getContext().getResources().getDimensionPixelOffset(R.dimen.dp_4));
                    }

                    mTextView.setY(complectedXPosition.get(i) - (mStepsViewIndicator.getCircleRadius() + mStepsViewIndicator.getCircleRadius() / 2));
