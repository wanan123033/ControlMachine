...
public class SwipeRefreshView extends SwipeRefreshLayout {
	...
	private boolean canLoadMore(){
		...
		if(mListView != null && mListView.getAdapter() != null){
			...
			if(mItemCount > 0 && itemCount >= mItemCount){
				// 总数只够1次加载
				isLastItem = false;
			}else{
				// 总数未设置,不滑动
				isLastItem = mListView.getLastVisiblePosition() == (itemCount - 1);
			}
		}
		if(isLastItem){
			Log.d(TAG,"------->  是最后一个条目");
		...
		return condition1 && isLastItem && !isLoading;
	}
	...
	public void setLoading(boolean loading){
		// 修改当前的状态
		isLoading = loading;
		//因为addFooterView(mFooterView)需要在setadapter之前调用
		//且可能存在removeFooterView时抛出异常,所以这里索性将mFooterView去掉
		//我们对此也没有刚性的需求
		if(isLoading){
			// 显示布局
			//mListView.addFooterView(mFooterView);
			//mListView.setAdapter(null);
			//Log.e("james","fuck,I am loading");
		}else{
			// 隐藏布局
